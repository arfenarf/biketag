...
def load_data(filepath="data/biketag.tsv", sep="\t"):
    ...
    gdf = gpd.GeoDataFrame(biketag, geometry=gpd.points_from_xy(biketag.longitude, biketag.latitude), crs="EPSG:4326", )
    return gdf
...
def create_person_df(gdf):
    ...
    person_df["convex_hull_area"] = (person_df.convex_hull.to_crs("+proj=cea").area * 3.861e-7)
    person_df["location_density"] = (person_df.location_count / person_df.convex_hull_area)
...
def merge_person_data(gdf, person_df):
    gdf = gdf.merge(person_df[["person_id"]], left_on="placed_by", right_index=True).rename(
        columns={"person_id": "placed_by_id"})
    gdf = gdf.merge(person_df[["person_id"]], left_on="found_by", right_index=True).rename(
        columns={"person_id": "found_by_id"})
    return gdf
...
def create_mapping_df(person_df):
    mapping_df = (person_df.loc[person_df.location_count > 2].reset_index().set_geometry("convex_hull").sort_values(
        by="convex_hull_area", ascending=False))
    return mapping_df


def build_map(map_df, person_df, gdf, peopleform=None):
    if peopleform:
        map_df = map_df.loc[map_df.person.isin(peopleform.options.data)]

    # viridis_colors = cm.linear.Set1_08.to_step(n=person_df.person_id.max()).scale(0, person_df.person_id.max())
    step = cm.linear.Set1_08.to_step(person_df.cluster.max()).scale(0, 5)
    map_df["__color"] = map_df.cluster.apply(step)

    # create the map object
    m = folium.Map(location=[42.3312832, -83.79662475], zoom_start=11, tiles=None)

    folium.TileLayer(name="OpenCycleMap",
    ...
    # add the hull shapes
    h = folium.GeoJson(map_df[["person", "convex_hull", "convex_hull_area", "cluster", "location_density", "__color", ]],
                   name="Rider Coverage Area",
                   style_function=lambda x: {"fillColor": x["properties"]["__color"], "color": "grey", "weight": 1,
                                             "fillOpacity": 0.25, },
                   tooltip=folium.GeoJsonTooltip(fields=["person", "cluster", "convex_hull_area", "location_density"],
                                                 aliases=["Person Name", "cluster", "Area (sq. mi.)",
                                                          "Tags/mi^2"], ), )
    h.add_to(m)
    ...
    if peopleform:
        display_gdf = gdf.loc[
            (gdf.placed_by.isin(peopleform.options.data)) | (gdf.found_by.isin(peopleform.options.data))]

    else:
        display_gdf = gdf.copy()

    if len(display_gdf) > 0:
        folium.GeoJson(display_gdf, name="Tags Placed",
                       marker=folium.CircleMarker(radius=3, fill_color="darkblue", fill_opacity=1, color="grey",
                                                  weight=1),
                       tooltip=folium.GeoJsonTooltip(fields=["name", "placed_by", "found_by"],
                                                     aliases=["Location Name", "Placed By", "Found By"], ),
                       popup=folium.GeoJsonPopup(fields=["name", "placed_by", "found_by"],
                                                 aliases=["Location Name", "Placed By", "Found By"], )).add_to(m)

    # add the centroids
    if peopleform:
        centroid_df = person_df.loc[peopleform.options.data]

    else:  # default to showing centroids
        centroid_df = person_df.copy()

    if len(centroid_df) > 0:
        centroid_df = centroid_df.set_geometry("convex_hull_center", drop=True)
        centroid_df = centroid_df[["cluster", "cluster_label", "person_id", 'geometry']].reset_index()
        centroid_df["__color"] = centroid_df.cluster.apply(step)
        z = folium.GeoJson(centroid_df, name="Centroids",
                           marker=folium.CircleMarker(radius=6, fill_color = 'green', fill_opacity=1, color="black", weight=1),
                           tooltip=folium.GeoJsonTooltip(fields=["person", "cluster_label"], aliases=["Centroid for:", "Cluster:"]),
                           popup=folium.GeoJsonPopup(fields=["person", "cluster_label"], aliases=["Centroid for:", "Cluster:"]),
                           style_function=lambda x: {"fillColor": x["properties"]["__color"], "color": "black",
                                                     "weight": 1,
                                                     "fillOpacity": 0.7, },)
