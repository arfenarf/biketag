...
def create_plotly_distograms(tag_df):
    ...
    fig.update_layout(
        barmode="overlay", title="Distance Histograms", xaxis_title="Distance (miles)"
    )
...
def create_plotly_heatmap(tag_df):
    heatmap_df = (
        tag_df.groupby(
            [tag_df.placed_at_datetime.dt.year, tag_df.placed_at_datetime.dt.month]
        )["name"]
        .count()
        .unstack(level=0)
        .fillna(0)
...
def create_bokeh_network(person_df, tag_df):
    ...
    for node in G.nodes:
        G.nodes[node]["x"] = (
            person_df.loc[person_df.person_id == node].convex_hull_center.x.values[0]
            / 10000
        )
        G.nodes[node]["y"] = (
            person_df.loc[person_df.person_id == node].convex_hull_center.y.values[0]
            / 10000
        )
        G.nodes[node]["name"] = person_df.loc[person_df.person_id == node].index[0]
        G.nodes[node]["cluster"] = person_df.loc[
            person_df.person_id == node
        ].cluster.values[0]
        G.nodes[node]["cluster_color"] = color_palette[
            person_df.loc[person_df.person_id == node].cluster.values[0]
        ]

    number_to_adjust_by = 5
    adjusted_node_size = dict(
        [(node, degree + number_to_adjust_by) for node, degree in nx.degree(G)]
    )
    ...
    plot.xaxis.major_tick_line_color = None  # turn off x-axis ticks
    plot.yaxis.major_tick_line_color = None  # turn off y-axis ticks
